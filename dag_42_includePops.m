function [sErr, mNorm, mID, params] = dag_42_includePops  (j, params, mNorm, mID,m2DHist, m2DHistS, vXA, vYA, vIParentPopRows)
...
try
  if                                              ~ isempty(vIParentPopRows)
    ...
    for                                         k = 1:length(vContourLevels)                                        % For each contour level
      m2DHistThres                                = m2DHistS > vContourLevels(k);                                   % Select data points that exceed contour level = object 
      [mBWLabel,iNBlobs]                          = bwlabel(m2DHistThres,4);                                        % Blob = At least 4 connected objects
                                                                                                                    % Blob = High density area
      stats                                       = regionprops(mBWLabel, 'area', 'centroid', 'boundingbox');
      for                                       l = 1:iNBlobs                                                       % Loop through each blob and find centroid and draw box around it
        ...
        for                                     m = 1:iNPopsToInclude                                               % For each blob, determine if blob is within the range specified by config file
          if                                        ( (vMinX(m)        <= iNewBlobCentX   ) && (iNewBlobCentX     <= vMaxX(m)) )  && ...
                                                    ( (vMinX(m)        <= iNewBlobBBoxMinX) && (iNewBlobBBoxMaxX  <= vMaxX(m)) )  && ...
                                                    ( (vMinY(m)        <= iNewBlobCentY   ) && (iNewBlobCentY     <= vMaxY(m)) )  && ...
                                                    ( (vMinY(m)        <= iNewBlobBBoxMinY) && (iNewBlobBBoxMaxY  <= vMaxY(m)) ) ;
